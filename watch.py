...
def handle_event(event):
  ...
  # UNISWAP TRADE, MATIC, WMATIC
    ...
    if farmbuy > 0:
      ...
      if pricechange < PRICEDELTA_PERCENT_THRESHOLD_MATIC:
        return
      msg = (f':chart_with_upwards_trend: At block `{blocknum}`, '
             f'`+{pricechange:.4f}%`:evergreen_tree: to `{price:,.2f} MATIC`; '
             f'`{farmbuy:,.2f}` iFARM was [bought](<https://explorer-mainnet.maticvigil.com/tx/{txhash}>)\n'
             f'by [{sender}](<https://explorer-mainnet.maticvigil.com/address/{sender}>)!'
             )
    if farmsell > 0:
      color = 16711680
      ...
      if pricechange < PRICEDELTA_PERCENT_THRESHOLD_MATIC:
        return
      msg = (f':chart_with_downwards_trend: At block `{blocknum}`, '
             f'`-{pricechange:.4f}%`:small_red_triangle_down: to `{price:,.2f} MATIC`; '
 ...
 # MATIC ROOT EVENT
  ...
  # MATIC ERC20 EVENT
    ...
    if txhash in txids_seen:
      return
    if event.args.amount < 400000000000000000000:
      return
...
def main():
  if LOOKBACK == 'True':
    ...
    if LOOKBACK_MATIC_TRADES == 'True':
      lookback_filters.append(matic_unipool_ifarm_matic_contract.events.Swap.createFilter(fromBlock=START_BLOCK))
      lookback_filters.append(matic_unipool_ifarm_quick_contract.events.Swap.createFilter(fromBlock=START_BLOCK))
